...
export async function POST(req: Request) {
  try {
    ...
    if (!docId || !entryId) {
      return NextResponse.json({ error: "Debes indicar cuadernillo y capítulo." }, { status: 400 });
    }
    ...
    if (!existingIds.includes(entryId)) return NextResponse.json({ error: "Capítulo no encontrado." }, { status: 404 });

    const byId = new Map<string, Record<string, unknown>>();
    entriesSnap.docs.forEach((d) => byId.set(d.id, d.data() as Record<string, unknown>));
    const ordered = entriesSnap.docs
      .map((d) => {
        const r = d.data() as Record<string, unknown>;
        const weekIndex = typeof r.weekIndex === "number" && Number.isFinite(r.weekIndex) ? Math.floor(r.weekIndex) : 0;
        return { id: d.id, weekIndex };
      })
      .sort((a, b) => (a.weekIndex || 0) - (b.weekIndex || 0) || a.id.localeCompare(b.id))
      .map((x) => x.id)
    ...
    const msg = err instanceof Error ? err.message : "No fue posible eliminar el capítulo.";
